<?php
	...
	// get overtime request>admin>all
	public function get_overtime_request_count2($employee_id,$fecha_inicial,$fecha_final) {
		$sql = 'SELECT * FROM `xin_attendance_time_request` where employee_id = ? and is_approved = ? and request_date_request >= ? and request_date_request <= ?';
		$binds = array($employee_id,2,$fecha_inicial, $fecha_final);
		$query = $this->db->query($sql, $binds);
		$result = $query->result();
		return $result;
	}
